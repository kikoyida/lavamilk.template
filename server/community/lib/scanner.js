...
async function next(state, { github, ai, now, githubId }) {
  ...
  if (s.phase === 'profile') {
    const profile = await github('/users/' + s.account);
    if (profile.type !== 'User') throw new Error('unsupportedAccount');
    if (normalizeAccount(profile.login) !== s.account) throw new Error('notFound');
    if (String(profile.id) !== String(githubId)) throw new Error('identityChanged');
    s.kind = profile.type;
    ...
    const rows = await github(path + s.account + '/repos?type=' + (s.kind === 'Organization' ? 'public' : 'owner') + '&sort=full_name&direction=asc&per_page=100&page=' + s.page);
    ...
    const result = await github('/search/' + (field === 'commits' ? 'commits' : 'issues') + '?q=' + encodeURIComponent(query)
    ...
    s.report.ai = await summarize(s.report, ai);
    s.phase = 'done';
